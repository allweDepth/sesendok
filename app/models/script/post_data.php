                    ...
                    switch ($jenis) {
                        ...
                        case 'harga_satuan':
                            switch ($tbl) {
                                ...
                                case 'input':
                                    $kodePosting = 'cek_insert';
                                    $kondisi = [['kd_proyek', '=', $kd_proyek], ['kode', '=', $kode, 'AND']];
                                    $set = [
                                        'kd_proyek' => $kd_proyek,
                                        'kode' => $kode,
                                        'jenis' => $jenis_basic_price,
                                        'uraian' => $uraian,
                                        'satuan' => $satuan,
                        ...
                        case 'informasi_umum':
                            switch ($tbl) {
                                case 'edit':
                                case 'input':
                                    $kodePosting = 'cekdouble_insert';
                                    $kondisi = [['kd_proyek', '=', $kd_proyek], ['kode', '=', $kode, 'AND']];
                                    $set = [
                                        'kd_proyek' => $kd_proyek,
                                        'kode' => $kode,
                                        'nomor_uraian' => $nomor_uraian,
                                        'uraian' => $uraian,
                                        'nilai' => $nilai,
                                        'satuan' => $satuan,
                                        'keterangan' => $keterangan,
                        ...
                        case 'analisa_alat_custom':
                            ...
                            switch ($tbl) {
                                case 'input':
                                    ...
                                    $hasil = $Fungsi->analisa($kd_proyek, $kd_analisa, $dataArrayHasil, $jenisImporData, $jenis);
                                    //hapus dahulu kd_analisa yang sama lalu insert 
                                    //var_dump($hasil);
                                    $jumlahArray = is_array($hasil) ? count($hasil) : 0;
                                    if ($jumlahArray) {
                                        $kondisi_delete = [['kd_proyek', '=', $kd_proyek], ['kd_analisa', '=', $kd_analisa, 'AND']];
                                        $data[$tabel_pakai] = $DB->delete_array($tabel_pakai, $kondisi_delete);
                                        $kodePosting = 'update_analisa';
                                    } else {
                                        $code = 41;
                                    }
                                    break;
                                case 'edit':
                                    ...
                                    $dataArrayHasil = [];
                                    //var_dump($dataArray);
                                    //usort($dataArray, array('MasterFungsi', 'sortinNo_Sortir'));
                                    //ksort($dataArray['no_sortir']);
                                    foreach ($dataArray as $key => $val) {
                                        $idKlmSamaAnalisa = 'NAN';
                                    ...
                                    //ksort($dataArrayHasil['no_sortir']);
                                    $hasil = $Fungsi->analisa($kd_proyek, $kd_analisa, $dataArrayHasil, $jenisImporData, $jenis);
                                    //var_dump($hasil);
                                    $jumlahArray = is_array($hasil) ? count($hasil) : 0;
                                    if ($jumlahArray) {
                        ...
                        case 'analisa_alat':
                            switch ($tbl) {
                                ...
                                case 'edit':
                                    ...
                                    $jumlahArray = is_array($sukuBunga_i) ? count($sukuBunga_i) : 0;
                                    $sukuBunga_i = $jumlahArray > 0 ? get_object_vars($sukuBunga_i[0])["nilai"] : 0;
                                    ...
                                    $jumlahArray = is_array($data) ? count($data) : 0;
                                    if ($jumlahArray > 0) {
                                        $code =  200;
                                    ...
                                    case 'harga_satuan':
                                        switch ($tbl) {
                                            case 'edit':
                                                ...
                                                if ($DB->count()) {
                                                    $Fungsi->hargaSatuan($kd_proyek, $id_row);
                                                }
                                                break;
                                            default:
                                                break;
