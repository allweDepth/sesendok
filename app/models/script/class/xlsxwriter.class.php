...
class XLSXWriter
{
	...
	public function writeSheetHeader($sheet_name, array $header_types, $col_options = null)
	{
		...
		if (!$suppress_row) {
			//tambahan untuk mengakomodir tinggi header
	...
	protected function writeCell(XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
	{
		...
		if (!is_scalar($value) || $value === '') { //objects, array, empty; null is not scalar
			...
			$file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="n"><v>' . (($value && $value !== ' ') ? intval(self::convert_date_time($value)) : '') . '</v></c>');
		} elseif ($num_format_type == 'n_datetime') {
			$file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="n"><v>' . (($value && $value !== ' ') ? self::convert_date_time($value) : '') . '</v></c>');
		} elseif (($num_format_type == 'n_currency') || ($num_format_type == 'n_numeric' && preg_match("/^((?:\d.\d{3}.|\d{1,3}.)?\d{1,3},\d{1,2})$/", $value))) {
			$value = str_replace('.', '', $value);
			$value = str_replace(',', '.', $value);
			$file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '"><v>' . $value . '</v></c>'); // override/force cell_style for currency
		} elseif ($num_format_type == 'n_numeric' || ($num_format_type == 'n_string' && is_numeric($value))) {
			$file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="n"><v>' . self::xmlspecialchars($value) . '</v></c>'); //int,float
		...
		} elseif ($num_format_type == 'n_auto' || 1) { //auto-detect unkdie()nown column types
			if (preg_match("/^((?:\d.\d{3}.|\d{1,3}.)?\d{1,3},\d{1,2})$/", $value)) {
				$value = str_replace('.', '', $value);
				$value = str_replace(',', '.', $value);
				$file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '"><v>' . $value . '</v></c>'); // override/force cell_style for currency
			} elseif (!is_string($value) || $value == '0' || ($value[0] != '0' && ctype_digit($value))) {
	...
	private static function determineNumberFormatType($num_format)
	{
		...
		if (preg_match('/\[\$\$\-1009\]|\[\$€\-407\]/i', $num_format)) return 'n_currency';
	...
	private static function numberFormatStandardized($num_format)
	{
		...
		else if ($num_format == 'percent')  $num_format = '#,##0.00_-"%"';
		$ignore_until = '';
		$escaped = '';
		for ($i = 0, $ix = strlen($num_format); $i < $ix; $i++) {
			...
			if ($ignore_until === '' && ($c === ' ' || $c === '-' || $c === '+' || $c === '(' || $c === ')') && ($i === 0 || $num_format[$i - 1] !== '_')) {
				$escaped .= "\\" . $c;
			} else {
				$escaped .= $c;
			}
	...
	public static function convert_date_time($date_input) //thanks to Excel::Writer::XLSX::Worksheet.pm (perl)
	{
		...
		} else if (preg_match("/(\d{2})\.(\d{2})\.(\d{4})/", $date_time, $matches)) {
			list($junk, $day, $month, $year) = $matches;
		}
		...
		return str_replace(',', '.', $days + $seconds);
	}
	//------------------------------------------------------------------
}
