...
class MasterFungsi
{
    ...
    public function laporanPeriodik($kd_proyek, $tanggal_awal, $tanggal_akhir)
    {
        $DB = DB::getInstance();
        $condition = [['kd_proyek', '=', $kd_proyek]];
        $DB->orderBy('no_sortir');
        $tabel_pakai = 'rencana_anggaran_biaya';
        $rowRab = $DB->getWhereCustom($tabel_pakai, $condition);
        ...
        if ($jumlahArray) {
            ...
            $jumlahHargaProyek = (float)$sum[0]->{'SUM(jumlah_harga)'}; //cara mengambil value object di array
            $DB->orderBy('id_rab');
            $tabel_pakai = 'monev';
            foreach ($rowRab as $keyRab => $valueRab) {
                $id = $valueRab->id;
                //realisasi sebelum periode
                $condition = [['kd_proyek', '=', $kd_proyek], ['id_rab', '=', $id, 'AND'], ['tanggal', '<', $tanggal_awal, 'AND']];
                $DB->select('SUM(realisasi_fisik) AS realisasi_fisik, SUM(realisasi_keu) AS realisasi_keu');
                $rowMonevSebelumPeriode = $DB->getWhereCustom($tabel_pakai, $condition);
                //realisasi saat periode
                $condition = [['kd_proyek', '=', $kd_proyek], ['id_rab', '=', $id, 'AND'], ['tanggal', '>=', $tanggal_awal, 'AND'], ['tanggal', '<=', $tanggal_akhir, 'AND']];
                $DB->select('SUM(realisasi_fisik) AS realisasi_fisik, SUM(realisasi_keu) AS realisasi_keu');
                $rowMonevPeriode = $DB->getWhereCustom($tabel_pakai, $condition);
                //var_dump($rowMonevPeriode);
                //data bobot
                $jumlahHarga = (float)$valueRab->jumlah_harga;
                $harga_satuan = (float)$valueRab->harga_satuan;
                $rowRab[$keyRab]->Bobot = $jumlahHarga / $jumlahHargaProyek * 100;
                //data realisasi
                //realisasi sebelum periode $tanggal_awal -> $tanggal_akhir
                $RealisasiFisikSebelum = (float)$rowMonevSebelumPeriode[0]->realisasi_fisik;
                $RealisasiKeuSebelum = (float)$rowMonevSebelumPeriode[0]->realisasi_keu;
                $rowRab[$keyRab]->RealisasiFisikSebelum = $RealisasiFisikSebelum;
                $rowRab[$keyRab]->RealisasiKeuSebelum = $RealisasiKeuSebelum;
                //realisasi periode ($tanggal_awal -> $tanggal_akhir)
                $RealisasiFisikPeriode = (float)$rowMonevPeriode[0]->realisasi_fisik;
                $RealisasiKeuPeriode = (float)$rowMonevPeriode[0]->realisasi_keu;
                $rowRab[$keyRab]->RealisasiFisikPeriode = $RealisasiFisikPeriode;
                $rowRab[$keyRab]->RealisasiKeuPeriode = $RealisasiKeuPeriode;
                //proggres volume sd periode ($tanggal_awal -> $tanggal_akhir)
                $proggVolSdPeriode_Fisik = $RealisasiFisikSebelum + $RealisasiFisikPeriode;
                $proggVolSdPeriode_Keu = $RealisasiKeuSebelum + $RealisasiKeuPeriode;
                //bobot tercapai
                $BtSdPeriodeLaluFisik = ($RealisasiFisikSebelum * $harga_satuan) / $jumlahHargaProyek * 100;
                $BtSdPeriodeLaluKeu = ($RealisasiKeuSebelum / $jumlahHargaProyek) * 100;
                $BtSdPeriodeIniFisik = ($RealisasiFisikPeriode * $harga_satuan) / $jumlahHargaProyek * 100;
                $BtSdPeriodeIniKeu = ($RealisasiKeuPeriode / $jumlahHargaProyek) * 100;
                //total bobot sampai $tanggal_akhir
                $TotalFisik = $BtSdPeriodeLaluFisik + $BtSdPeriodeIniFisik;
                $TotalKeu = $BtSdPeriodeLaluKeu + $BtSdPeriodeIniKeu;
                //rekam di data rowRab
                $rowRab[$keyRab]->proggVolSdPeriode_Fisik = $proggVolSdPeriode_Fisik;
                $rowRab[$keyRab]->proggVolSdPeriode_Keu = $proggVolSdPeriode_Keu;
                $rowRab[$keyRab]->BtSdPeriodeLaluFisik = $BtSdPeriodeLaluFisik;
                $rowRab[$keyRab]->BtSdPeriodeLaluKeu = $BtSdPeriodeLaluKeu;
                $rowRab[$keyRab]->BtSdPeriodeIniFisik = $BtSdPeriodeIniFisik;
                $rowRab[$keyRab]->BtSdPeriodeIniKeu = $BtSdPeriodeIniKeu;
                $rowRab[$keyRab]->TotalFisik = $TotalFisik;
                $rowRab[$keyRab]->TotalKeu = $TotalKeu;
            }
        }
        return $rowRab;
    }
    public function analisa($kd_proyek, $kd_analisa, $DataRows, $jenis = 'koef', $jns = '') //jenis koefisien atau harga satuan// dibedakan menurut kolom rumus untuk koef/harga satuan
    {
        //$real_path = realpath(dirname(__FILE__));
        //var_dump(scandir('../app/models/script/class/'));
        require_once("FormulaParser.php");
        require_once('../app/models/script/init.php');
        ...
        $sum = 0;
        //var_dump($DataRows);
        $DataRows = json_decode(json_encode($DataRows), TRUE);

        usort($DataRows, array('MasterFungsi', 'sortinNo_Sortir')); //sortir menurut no sortir
        $tampungArrayDataRows = [];
        $sum_Jumlah_harga = 0;
        //var_dump($DataRows);
        //var_dump("jns : $jns");
        foreach ($DataRows as $key => $value) {
            $validateRow = new Validate($value);
            $nomor = $validateRow->setRules('nomor', 'Nomor', [
                'sanitize' => 'string'
            ]);
            $uraian = $validateRow->setRules('uraian', 'Uraian', [
                'sanitize' => 'string',
                'required' => true,
                'min_char' => 1
            ]);
            $kode = $validateRow->setRules('kode', 'Kode Uraian', [
                'sanitize' => 'string'
            ]);
            $satuan = $validateRow->setRules('satuan', 'Satuan', [
                'sanitize' => 'string'
            ]);
            $value['koefisien'] = floatval($value['koefisien']) ?: 0;
            $koefisien = $validateRow->setRules('koefisien', 'Koefisien', [
                'numeric' => true
            ]);
            $value['harga_satuan'] = floatval($value['harga_satuan']) ?: 0;
            $harga_satuan = $validateRow->setRules('harga_satuan', 'Harga Satuan', [
                'numeric' => true
            ]);
            $rumus = $validateRow->setRules('rumus', 'Rumus', [
                'sanitize' => 'string'
            ]);
            $keterangan = $validateRow->setRules('keterangan', 'Keterangan', [
                'sanitize' => 'string'
            ]);
            if ($validateRow->passed()) {
                ...
                $koefisien = (float)$value['koefisien'];
                $satuan = $value['satuan'];
                $rumus = $value['rumus'];
                */
                //var_dump("koefisien $kode : $koefisien");
                //kumpulkan kode
                $rumusTampung = $rumus;
                if (strlen($rumus) > 0) {
                    // hitung dulu karakter "(" dan ")" jika ada maka jumlahnya harus sama
                    $countDalamKurung = substr_count($rumus, '(');
                    $countBalasKurung = substr_count($rumus, ')');
                    if ($countDalamKurung == $countBalasKurung) {
                        $hasil = preg_match("/^(0-9\s)$/", $rumus);
                        $rumus = str_replace('{', '(', $rumus);
                        $rumus = str_replace('}', ')', $rumus);
                        $rumus = str_replace(':', '/', $rumus);
                        $rumus = str_replace('x', '*', $rumus);
                        $rumus = str_replace('X', '*', $rumus);
                        $rumus = str_replace(',', '.', $rumus);
                        //var_dump("$rumus ");
                        $rumusTampung = $rumus;
                        foreach ($tampungArrayDataRows as $baris) {
                            ...
                            $bariskoef = $baris['koefisien'];
                            if ($baris['harga_satuan'] > 0) {
                                $rumus = str_replace($baris['kode'], $baris['koefisien'] * $baris['harga_satuan'], $rumus);
                            } else {
                                $rumus = str_replace($baris['kode'], $baris['koefisien'], $rumus);
                            }
                        }
                        try {
                            $parser = new FormulaParser($rumus);
                            $result = $parser->getResult();
                            //var_dump("$rumus : $result[1] : kode =$kode");
                            if ($result[1] !== 'Syntax error' && is_nan($result[1]) === false) {
                                switch ($jenis) {
                                    case 'koef':
                                        $koefisien = (float)$result[1];
                                        break;
                                    case 'harga':
                        ...
                        $rumus .= ' jumlah karakter "()"(dalam kurung) tidak sama/(koreksi jumlah karakter "(" dan ")"..!!)';
                    }
                }
                $jenis_kode = null;
                //cari basic price
                ...
                if (count((array)$get_data) > 0) {
                    switch ($jenis) {
                        case 'koef':
                            $koefisien = $get_data[0]->harga_satuan;
                            break;
                        case 'koef_harga':
                        case 'harga':
                    ...
                    if (count((array)$get_data) > 0) {
                        switch ($jenis) {
                            case 'koef':
                                $koefisien = $get_data[0]->koefisien;
                                break;
                            case 'koef_harga':
                            case 'harga':
                        ...
                        if (count((array)$get_data) > 0) {
                            switch ($jenis) {
                                case 'koef':
                                    $koefisien = $get_data[0]->total_biaya_sewa;
                                    break;
                                case 'koef_harga':
                                case 'harga':
                ...
                //var_dump("jenis_kode: $jenis_kode");
                //var_dump("baris akhir $kode : $koefisien");
                $koefisien = (is_nan($koefisien)) ? 0 : $koefisien;
                //var_dump("baris akhir2b $kode : $koefisien");
                $koefisien = ($koefisien == 'INF') ? 0 : $koefisien;
                //var_dump("baris akhir2c $kode : $koefisien");
                //var_dump("koefisien $kode ,$rumus = $koefisien");
                $jumlah_harga = (float)$koefisien * (float)$harga_satuan;
                //jika nomor == '>>' jumlahkan jumlah harga
                if ($nomor == '>>' || $jenis == 'harga') {
                    $sum_Jumlah_harga += $jumlah_harga;
                    // var_dump("sum_Jumlah_harga $kd_analisa:$sum_Jumlah_harga");

                }

                //jika nomor == kd_analisa dan tidak mempunyai rumus hasil penjumlahan >> di taruh di koefisien
                //var_dump("nomor = $nomor : kode analisa = $kd_analisa");
                //var_dump("koefisien : $kode <> $kd_analisa");

                if (($nomor == $kd_analisa || $kode == $kd_analisa) && strlen($rumus) <= 0) {
                    if ($jns != 'analisa_quarry') {
                        //var_dump("jns : $jns");
                        $koefisien = $sum_Jumlah_harga;
                        //apakah harus merubah rencana_anggaran_biaya disini?
                    }
                }
                //var_dump("jenis_kode: $jenis_kode");
                //var_dump($koefisien);
                ...
                //var_dump("baris $key akhir kode $kode : $koefisien");
                if (strlen($kode) > 0 && $kode != 0) {
                    //var_dump($kode);
                    $tampungArrayDataRows[$kode] = $arrayDataRows;
                    $jumlahArray = is_array($tampungArrayDataRows) ? count($tampungArrayDataRows) : 0;
                    //usort($tampungArrayDataRows, fn($a, $b) => strcmp($a->kode, $b->nkode));
                    usort($tampungArrayDataRows, array('MasterFungsi', 'sortin'));
                    //var_dump('$tampungArrayDataRows');
                    //var_dump($tampungArrayDataRows);
                }
                //$DataRows[$key]['kd_proyek'] = $kd_proyek;
                //$DataRows[$key]['kd_analisa'] = $kd_analisa;
                #==========================
                #==data asli yang dirubah==
                #==========================
                //var_dump("$jns baris akhir $kode : $koefisien, harga satuan: $harga_satuan");
                $DataRows[$key]['rumus'] = $rumusTampung;
                $DataRows[$key]['koefisien'] = (float)$koefisien;
                $DataRows[$key]['harga_satuan'] = (float)$harga_satuan;
                $DataRows[$key]['jumlah_harga'] = (float)$jumlah_harga;
                $DataRows[$key]['jenis_kode'] = $jenis_kode;
                // total jumlah harga
                //var_dump("baris2 akhir $kode : {$DataRows[$key]['koefisien']}, harga satuan: $harga_satuan");
                if (($kode == $kd_analisa || $kd_analisa == $nomor && strlen($rumus) <= 0) && $jns != 'analisa_quarry') {
                    $DataRows[$key]['koefisien'] = $sum_Jumlah_harga;
                }
                //var_dump("baris3 akhir $kode : {$DataRows[$key]['koefisien']}, harga satuan: $harga_satuan");
            } else {
                $DataRows['error'][$key] = $validateRow->getError();
            }
        }
        //ganti koefisien dengan data baru
        //var_dump($tampungArrayDataRows);
        //var_dump($DataRows);
        return $DataRows;
    }
    ...
    public function getTabel($jenis, $nama_tabel = '', $get_data = [], $jmlhalaman = 0, $halaman = 1, $jumlah_kolom = 1, $type_user = 'user')
    {
        ...
        $rowData = ['tbody' => '', 'tfoot' => ''];
        if ($jenis == 'lap-harian') {
            $rowDataTemp = ['tbodyUpah' => '', 'tfootUpah' => '', 'tbodyBahan' => '', 'tfootBahan' => '', 'tbodyPeralatan' => '', 'tfootPeralatan' => '', 'tbodyCuaca' => '', 'tfootCuaca' => '', 'tbodyNote' => '', 'tfootNote' => ''];
        }
        ...
        if ($jumlahArray > 0) {
            ...
            switch ($jenis) {
                ...
                case 'analisa_alat_custom':
                    $rowData['tbody'] = '<div class="ui scrolling container"><table class="ui first head foot stuck unstackable mini compact celled table" tbl="' . $jenis . '"><thead><tr' . $classRow . '><th class="one wide">No.</th><th class="five wide">URAIAN</th><th>KODE</th><th>KOEF.</th><th>SATUAN</th><th>HARGA SATUAN</th><th>RUMUS KOEF.</th><th>KET</th><th class="center aligned collapsing"><div class="ui mini basic icon buttons"><button class="ui button" name="add_row" jns="' . $jenis . '" tbl="add_row_tabel" data-tooltip="add row" data-position="left center" data-inverted><i class="plus icon"></i></button><label jns="' . $jenis . '" tbl="add_row_tabel" for="invisibleupload1" class="ui button" data-tooltip="ambil data excel" data-position="left center" data-inverted><i class="excel green file outline icon"></i></label></div></th></tr></thead><tbody>';
                    break;
                case 'analisa_ck':
                    $rowData['tbody'] = '<div class="ui scrolling container"><table class="ui first head foot stuck unstackable mini compact celled table" tbl="' . $jenis . '"><thead><tr' . $classRow . '><th class="five wide">URAIAN</th><th>KODE</th><th>SATUAN</th><th>KOEF.</th><th>HARGA SATUAN</th><th>JUMLAH</th><th>RUMUS HARGA SATUAN</th><th class="center aligned collapsing"><div class="ui mini basic icon buttons"><button class="ui button" name="add_row" jns="' . $jenis . '" tbl="add_row_tabel" data-tooltip="add row" data-position="left center" data-inverted><i class="plus icon"></i></button><label jns="' . $jenis . '" tbl="add_row_tabel" for="invisibleupload1" class="ui button" data-tooltip="ambil data excel" data-position="left center" data-inverted><i class="excel green file outline icon"></i></label></div></th></tr></thead><tbody>';
                    break;
                case 'user':
                    break;
                default:
    ...
    #saat harga_satuan di tabel harga_sat_upah_bahan dirubah
    public function hargaSatuan($kd_proyek, $id_hargaSatuan)
    {
        $DB = DB::getInstance();
        //analisa_alat_custom
        // cari di tabel 'analisa_alat_custom','analisa_alat','analisa_quarry','analisa_pekerjaan_bm','analisa_pekerjaan_ck','analisa_pekerjaan_sda'
        // yang ada kode 
        $tabel = ['analisa_alat_custom', 'analisa_alat', 'analisa_quarry', 'analisa_pekerjaan_bm', 'analisa_pekerjaan_ck', 'analisa_pekerjaan_sda'];
        $rowHargaSatuan = $DB->getWhereCustom('harga_sat_upah_bahan', [['kd_proyek', '=', $kd_proyek], ['id', '=', $id_hargaSatuan, 'AND']]);
        $jumlahArray = is_array($rowHargaSatuan) ? count($rowHargaSatuan) : 0;
        if ($jumlahArray) {
            foreach ($rowHargaSatuan[0] as $key => $value) {
                ${$key} = $value;
                // var_dump($key);
            }
            // var_dump("harga_satuan : $harga_satuan");
            // ambil data persentasi operasional dan keuntungan $op di informasi umum
            $op = 0;
            ...
            foreach ($tabel as $key => $value) {
                ...
                $condition = [['kd_proyek', '=', $kd_proyek], ['kode', '=', $kode, 'AND']];
                $columnName = 'kode';
                //ambil kolom sesuai tabel
                switch ($tableName) {
                    case 'analisa_alat':
                        $columnName = 'kode';
                        break;
                    default:
                        $columnName = 'kd_analisa';
                        break;
                }
                //list dari kd_analisa yang ada harga satuan berubah
                $rowList = $DB->getDistinct($tableName, $columnName, $condition);
                $jumlahArray = is_array($rowList) ? count($rowList) : 0;
                if ($jumlahArray) {
                    //var_dump($tableName);
                    //var_dump($rowList);//list dari kd_analisa yang ada harga satuan berubah
                    //hitung kode analisa atau kode untuk analisa_alat
                    foreach ($rowList as $keyRowList => $valueRowList) {
                        $kd_analisa = $valueRowList->kd_analisa; //$rowList[0]->kd_analisa;
                        $tbl = 'analisa_bm';
                        switch ($tableName) {
                            case 'analisa_pekerjaan_sda':
                                $tbl = 'analisa_sda';
                            case 'analisa_pekerjaan_bm':
                            ...
                            case 'analisa_alat_custom':
                                $jenisImporData = 'koef';
                                $tbl = 'analisa_alat_custom';
                                break;
                            case 'analisa_pekerjaan_ck':
                        ...
                        $DB->orderBy('no_sortir');
                        $dataArray = $DB->getWhereCustom($tableName, $condition);
                        $jumlahArray = is_array($dataArray) ? count($dataArray) : 0;
                        if ($jumlahArray) {
                            $keys = array_column($dataArray, 'id'); // kumpulkan key 
                            $dataArrayHasil = [];
                            ...
                            if ($idKlmSamaDenganAnalisa == 'NAN') {
                                ...
                                switch ($tableName) {
                                    case 'analisa_pekerjaan_ck':
                                        $jenis_kodeInsert = 'summary:' . $kd_analisa;
                                        break;
                                    case 'analisa_alat_custom':
                            ...
                            $hasil = $this->analisa($kd_proyek, $kd_analisa, $dataArrayHasil, $jenisImporData, $tbl);
                            //var_dump($hasil);
                            ...
                            if ($jumlahArray) {
                                ...
                                foreach ($hasil as $key => $val) {
                                    ...
                                    if ($id_rows) {
                                        ...
                                        $set = $val_update;
                                        $sumRows = $DB->getWhereArray($tableName, $kondisi);
                                    }
                                    $jumlahArray = is_array($sumRows) ? count($sumRows) : 0;
                                    if ($jumlahArray) {
                                        $resul = $DB->update_array($tableName, $set, $kondisi);
                                        //var_dump($DB->count());
                                        ...
                                        if ($DB->count()) {
                                            ...
                                            /* 
                                            jika kode === kd_analisa analisa untuk ck, nomor==kd_analisa untuk analisa lainnya
                                            ubah tabel rencana_anggaran_biaya yang mempunyai kd_analisa yang sama  
                                            */
                                            if ($kd_analisa == $kodeRows || $kd_analisa == $nomorRows) {
                                                //var_dump($kd_analisa );
                                                $kondisiRAB = [['kd_proyek', '=', $kd_proyek], ['kd_analisa', '=', $kd_analisa, 'AND']];
                                                $dataArrayRAB = $DB->getWhereCustom('rencana_anggaran_biaya', $kondisiRAB);
                                                $jumlahArray = is_array($dataArrayRAB) ? count($dataArrayRAB) : 0;
                                                //var_dump($dataArrayRAB );
                                                if ($jumlahArray) {
                                                    //hitung kembali rab
                                                    foreach ($dataArrayRAB as $keyRAB => $valRAB) {
                                                        $idRAB = $valRAB->id;
                                                        $volumeRAB = $valRAB->volume;
                                                        $kondisiRABup = [['kd_proyek', '=', $kd_proyek], ['kd_analisa', '=', $kd_analisa, 'AND'], ['id', '=', $idRAB, 'AND']];
                                                        $jumlah_op = $hargaDasarRows * ($op / 100);
                                                        $harga_satuan = $jumlah_op + $hargaDasarRows;
                                                        $jumlah_harga = $harga_satuan * $volumeRAB;
                                                        $setRAB = [
                                                            'harga_dasar' => $hargaDasarRows,
                                                            'jumlah_op' => $jumlah_op,
                                                            'harga_satuan' => $harga_satuan,
                                                            'jumlah_harga' => $jumlah_harga
                                                        ];
                                                        //var_dump($setRAB);
                                                        $resul = $DB->update_array('rencana_anggaran_biaya', $setRAB, $kondisiRABup);
                                                    }
                                                }
                                            }
                                            ...
                                            if ($jenis == 'analisa_alat_custom') {
                                                ...
                                                if ($nomor_row == $kd_analisa) { //if ($nomor_row == $id) {
                                        ...
                                        $val_insert = $val;
                                        unset($val_insert["id"]);
                                        $resul = $DB->insert($tableName, $val_insert);
                                        //$jumlahArray = is_array($DB->count()) ? count($DB->count()) : 0;
                                        if ($DB->count()) {
                                            $data['add_row'][] = $key;
    ...
    #ubah analisa alat standar
    public function analisaAlat($kd_proyek, $DataRows, $sukuBunga_i, $M22, $M21, $L04, $L05)
    {
        require_once("FormulaParser.php");
        //var_dump($kd_proyek);
        //var_dump(json_decode(json_encode($DataRows), TRUE));
        //object ke array
        $DataRows = json_decode(json_encode($DataRows), TRUE);
        //var_dump($DataRows);
        //var_dump($sukuBunga_i);
        //var_dump($M22);
        //var_dump($M21);
        //var_dump($L04);
        //var_dump($L05);
        $jenis_peralatan = $DataRows['jenis_peralatan'];
        $kode = $DataRows['kode'];
        $tenaga = $DataRows['tenaga'];
        $kapasitas = $DataRows['kapasitas'];
        $sat_kapasitas = $DataRows['sat_kapasitas'];
        $harga_pakai = $DataRows['harga_pakai'];
        $umur = $DataRows['umur'];
        $jam_kerja_1_tahun = $DataRows['jam_kerja_1_tahun'];
        $ket_alat = $DataRows['ket_alat'];
        $ketentuan_tambahan = $DataRows['ketentuan_tambahan'];
        //var_dump($ketentuan_tambahan);
        //Mulai hitung alat
        $jumlah_operator = $DataRows['jumlah_operator'];
        $jumlah_pembantu_ope = $DataRows['jumlah_pembantu_ope'];
        $bahan_bakar2 = 0;
        $bahan_bakar1 = 0;
        $bahan_bakar3 = 0;
        $upah_operator = 0;
