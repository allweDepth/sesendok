...
class Input{
  ...
  public static function runSanitize($value,$sanitizeType){
    switch($sanitizeType) {
      case 'string':
        //$sanitizeValue = filter_var($value, FILTER_SANITIZE_STRING); deprecated php 8.1
        $sanitizeValue = filter_var($value, FILTER_UNSAFE_RAW);
        $sanitizeValue = preg_replace('/\x00|<[^>]*>?/', '', $sanitizeValue);
        $sanitizeValue = str_replace(["'", '"'], ['&#39;', '&#34;'], $sanitizeValue);
      break;
      case 'int':
